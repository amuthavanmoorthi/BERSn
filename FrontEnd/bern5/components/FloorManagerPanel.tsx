...
const FloorManagerPanel: React.FC<FloorManagerPanelProps> = ({
  ...
  const inputClass = "p-1.5 bg-white/10 border border-white/10 rounded-lg text-[13px] font-bold text-white focus:bg-white focus:text-slate-900 outline-none transition-all text-center w-full";
  const labelClass = "text-[12px] font-black text-slate-400 uppercase tracking-wide";
  ...
  const updateShapeParams = (floorId: string, shapeId: string, paramUpdates: Partial<FloorShape['params']>) => {
    onFloorsChange(floors.map(f => {
      ...
      // Keep floor height in sync with polyline extrudeHeight
      if (paramUpdates.extrudeHeight != null) {
        const editedShape = f.shapes.find(s => s.id === shapeId);
        if (editedShape?.type === 'polyline') {
          next.floorHeight = paramUpdates.extrudeHeight;
          next.shapes = next.shapes.map(s =>
            s.type === 'polyline' && s.id !== shapeId
              ? { ...s, params: { ...s.params, extrudeHeight: paramUpdates.extrudeHeight } }
              : s
          );
        }
      }
        ...
        return (
          <>
            ...
            {p.points && p.isClosed && p.points.length >= 3 && (
              <div className="col-span-3 space-y-1 pt-1 border-t border-white/5">
                <div className="text-[11px] font-black text-slate-400 uppercase tracking-wide">{t ? '邊長 / 夾腳' : 'Edges / Angles'}</div>
                  ...
                  {p.points.map((_pt, vi) => {
                    ...
                    return (
                      <div key={vi} className="flex items-center gap-1.5 text-[11px]">
                        <span className="font-mono text-slate-500 w-6 shrink-0">P{vi + 1}</span>
                        ...
                        <input
                          ...
                          className="w-14 px-1 py-0.5 bg-white/5 border border-white/10 rounded text-white font-mono text-center outline-none focus:bg-white/10"
                        />
                        ...
                        <input
                          ...
                          className="flex-1 min-w-0 px-1 py-0.5 bg-white/5 border border-white/10 rounded text-white font-mono text-center outline-none focus:bg-white/10"
                        />
                        <span className="text-slate-500 text-[10px] shrink-0">m</span>
                        <button
                          onClick={() => {
        ...
        {[...floors].reverse().map((floor, reverseIdx) => {
          ...
          return (
            ...
            >
              ...
              >
                {/* Expand arrow */}
                <span className={`text-[12px] text-slate-500 transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`}>
                  ▶
                </span>
                ...
                <div className="flex items-center gap-0.5 ml-auto mr-1.5 shrink-0">
                  <input
                    ...
                    className="bg-white/5 border border-white/10 rounded text-[11px] font-bold text-white w-10 text-center outline-none focus:bg-white/10 py-0.5"
                  />
              ...
              {isExpanded && (
                <div className="px-2.5 pb-2.5 space-y-2 animate-in slide-in-from-top-1 duration-200">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className="text-[13px] font-black text-slate-500">#{shapeIdx + 1}</span>
                            <select
                              value={shape.type}
                              onChange={(e) => updateShape(floor.id, shape.id, { type: e.target.value as GeometryType })}
                              onClick={(e) => e.stopPropagation()}
                              className="bg-white/5 border border-white/10 rounded-lg text-[12px] font-bold text-white px-2 py-1 outline-none focus:bg-white/10 cursor-pointer"
                            >
                          ...
                          <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                            <button onClick={() => duplicateShape(floor.id, shape.id)} className={`${btnSmClass} bg-white/5 text-blue-400 hover:bg-blue-600/20 !w-6 !h-6 text-[12px]`} title={t ? '複製' : 'Copy'}>⧉</button>
                            <button onClick={() => deleteShape(floor.id, shape.id)} className={`${btnSmClass} bg-white/5 text-red-400 hover:bg-red-600/20 !w-6 !h-6 text-[12px] ${floor.shapes.length <= 1 ? 'opacity-30 pointer-events-none' : ''}`} title={t ? '刪除' : 'Del'}>✕</button>
                          </div>
                        </div>
                        ...
                        {isShapeSelected && (
                          <div className="space-y-2 animate-in fade-in duration-200">
                            ...
                            <div className="flex items-center gap-2 p-2 rounded-lg bg-purple-500/10 border border-purple-500/20">
                              ...
                              />
                              <span className="text-[11px] text-slate-400 font-mono flex-1">{shape.params.color || '#ffffff'}</span>
                              {shape.params.color && (
                                <button
                                  onClick={() => updateShapeParams(floor.id, shape.id, { color: undefined })}
                                  className="text-[10px] px-2 py-0.5 rounded bg-white/5 text-slate-400 hover:bg-white/10"
                                  title={t ? '清除' : 'Clear'}
                                >✕</button>
                              )}
                            </div>
                  ...
                  <button
                    ...
                    className="w-full py-2 rounded-lg border border-dashed border-white/10 text-[12px] font-bold text-slate-500 hover:border-white/20 hover:text-white transition-all flex items-center justify-center gap-1"
                  >
...
const NoWindowFaceToggles: React.FC<{
  ...
  return (
    <div className="space-y-1 p-2 rounded-lg bg-slate-700/30 border border-white/10">
