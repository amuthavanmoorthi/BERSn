        ...
        <div className="space-y-3 overflow-y-auto max-h-[calc(100vh-300px)] pr-1 custom-scrollbar">
            ...
            <section className={sectionClass}>
                ...
                <div className="flex justify-between text-[8px] text-slate-400 px-1">
                    <span>{t ? '空調節能效率 EAC' : 'HVAC Efficiency EAC'}</span>
            ...
            <section className={sectionClass}>
                ...
                <div className="flex justify-between text-[8px] text-slate-400 px-1">
                    <span>{t ? '照明節能效率 EL' : 'Lighting Efficiency EL'}</span>
            ...
            <section className={sectionClass}>
                ...
                <div className="flex justify-between text-[8px] text-slate-400 px-1">
                    <span>{t ? '電梯節能效率 Et' : 'Elevator Efficiency Et'}</span>
            ...
            <section className={sectionClass}>
                ...
                <div className="flex justify-between text-[8px] text-slate-400 px-1">
                    <span>{t ? '熱水節能效率 EHW' : 'Hot Water Efficiency EHW'}</span>
            ...
            <section className={sectionClass + " bg-slate-900 text-white border-slate-700"}>
                ...
                <div className="space-y-1">
                    {[
                        { label: t ? '空調基準 AEUI' : 'HVAC Baseline AEUI', value: euiData.AEUI, unit: 'kWh/m²·yr', color: 'text-blue-400' },
                        { label: t ? '照明基準 LEUI' : 'Lighting Baseline LEUI', value: euiData.LEUI, unit: 'kWh/m²·yr', color: 'text-emerald-400' },
                        { label: t ? '其他用電基準 EEUI' : 'Equipment Baseline EEUI', value: euiData.EEUI, unit: 'kWh/m²·yr', color: 'text-orange-400' },
                        { label: t ? '外殼節能率 Es' : 'Envelope Saving Rate Es', value: esValue, unit: '', color: 'text-slate-300' },
                        { label: t ? '空調效率 EAC' : 'HVAC EAC', value: HVAC_SYSTEMS.find(s => s.id === selectedHVAC)?.defaultEAC || 1.0, unit: '', color: 'text-cyan-400' },
                        { label: t ? '照明效率 EL' : 'Lighting EL', value: LIGHTING_SYSTEMS.find(s => s.id === selectedLighting)?.defaultEL || 1.0, unit: '', color: 'text-yellow-400' },
                        { label: t ? '電梯效率 Et' : 'Elevator Et', value: ELEVATOR_TYPES.find(t => t.id === selectedElevator)?.EtValue || 1.0, unit: '', color: 'text-purple-400' },
                        { label: t ? '熱水效率 EHW' : 'Hot Water EHW', value: DHW_SYSTEMS.find(s => s.id === selectedDHW)?.EHW || 0, unit: '', color: 'text-red-400' },
                    ].map(item => (
