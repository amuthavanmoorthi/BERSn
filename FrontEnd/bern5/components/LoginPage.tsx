...
interface LoginPageProps {
    ...
    onPasskeyLogin: (username: string, rememberMe: boolean) => Promise<void>;
    lang: 'zh' | 'en';
    notice?: string;
}

const LoginPage: React.FC<LoginPageProps> = ({ onLogin, onPasskeyLogin, lang, notice }) => {
    const t = lang === 'zh';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : (t ? '登入失敗，請稍後再試。' : 'Login failed. Please try again.'));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handlePasskeyLogin = async () => {
        if (!username.trim()) {
            setError(t ? '請先輸入帳號或電子郵件，再使用 passkey。' : 'Enter your username or email first, then use a passkey.');
            return;
        }
        ...
        setError('');
        try {
            await onPasskeyLogin(username, rememberMe);
        } catch (err) {
            setError(err instanceof Error ? err.message : (t ? 'Passkey 登入失敗，請稍後再試。' : 'Passkey sign-in failed. Please try again.'));
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <div className="w-full lg:w-1/2 p-12 flex flex-col justify-center bg-white/50">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5 max-w-sm mx-auto w-full">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder={t ? '帳號或電子郵件' : 'Username or email'}
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className="w-full py-4 bg-white border-2 border-emerald-200 hover:border-emerald-300 text-emerald-700 font-bold text-base rounded-xl shadow-sm hover:shadow-md transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 9a2 2 0 10-4 0v1H8a2 2 0 00-2 2v2a4 4 0 108 0v-2a2 2 0 00-2-2V9zm6 3v2a8 8 0 11-16 0v-2" />
                            </svg>
                            <span>{t ? '使用 Passkey 登入' : 'Sign in with passkey'}</span>
                        </button>

                        <p className="text-xs text-slate-500 text-center">
                            {t ? 'Passkey 需先由系統管理員建立帳號，並在成功登入後於瀏覽器中註冊。' : 'Passkeys require an existing account and must be registered in this browser after a successful sign-in.'}
                        </p>

                        {notice && (
                            <p className="text-sm text-emerald-700 font-medium text-center bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3">
                                {notice}
                            </p>
                        )}
                    ...
                    <div className="mt-8 text-center">
                        <p className="text-xs text-slate-500 mb-2">
                            {t ? '請使用由系統管理員建立的帳號登入' : 'Sign in with an account provisioned by your system administrator'}
                        </p>
