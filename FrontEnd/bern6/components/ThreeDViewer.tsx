...
// ExtrudeHeightDialog removed — useDrawingTool auto-commits shapes with default
// height & extras. Kept the file for potential future "advanced create" toggle.
...
import { buildBakedGroup, bakeShapeGroup, applyBboxDeform } from '../lib/meshDeform';
...
// Effective story height for stacking = max(floor.floorHeight, tallest shape
// extrusion). Lets per-shape extrusions push the upper floors up to clear the
// tallest mass, while shorter siblings stay at their own heights (no overlap).
function effectiveFloorHeight(f: Floor): number {
  const base = f.floorHeight || 0;
  let maxShape = 0;
  for (const s of f.shapes || []) {
    const h = s.type === 'polyline'
      ? ((s.params as any).extrudeHeight ?? base)
      : ((s.params as any).height ?? base);
    if (h > maxShape) maxShape = h;
  }
  return Math.max(base, maxShape);
}
...
interface ThreeDViewerProps {
  ...
  /** Optional layer visibility map. When provided, each entry controls the
   *  visibility of shapes assigned to that layer (see `layerKeyForShapeType`
   *  in components/workspace/LayerPanel.tsx — polyline → 'planes', others →
   *  'mainModel'). Layers omitted from the map default to visible. */
  layerVisibility?: { mainModel: boolean; planes: boolean };
  /**
   * When true, the DrawingToolbar is rendered in its own flex column to the
   * LEFT of the 3D canvas (used by the new workspace shell — toolbar lives
   * between the dashboard panel and the 3D area, not overlaying the canvas).
   * When false/omitted, legacy overlay positioning.
   */
  inlineToolbar?: boolean;
  /** Default facade params for newly drawn shapes — sourced from STEP 3 Envelope. */
...
const ThreeDViewer: React.FC<ThreeDViewerProps> = ({
  ...
  onUndo, onRedo, canUndo, canRedo, topViewRequestSeq, layerVisibility, inlineToolbar, facadeDefaults,
}) => {
  ...
  // 變形 (bbox-cage deform) overlay — wireframe cage + 4 sphere handles around
  // the selected shape's XZ bbox at the active floor base. Rebuilt by the
  // `deformOverlayEffect` whenever tool / selection / shape bbox changes.
  const deformGroupRef = useRef<THREE.Group | null>(null);
  // Vertex-normal helpers — one per child mesh of the selected shape. Active
  // whenever the 變形 tool is on so users can see how the bake's normals
  // (recomputed after each cage drag) point. Tear down on tool/selection change.
  const normalsHelpersRef = useRef<VertexNormalsHelper[]>([]);
  // Live state during a handle drag — null = no active deform.
  const deformActiveRef = useRef<{
    shapeId: string;
    floorId: string;
    axis: '+x' | '-x' | '+z' | '-z';
    startBboxXZ: { minX: number; maxX: number; minZ: number; maxZ: number };  // bbox snapshot at drag start
    startCursorXZ: { x: number; z: number };                                   // world cursor at drag start
  } | null>(null);
  const [snapToGrid, setSnapToGrid] = useState(true);
  // CAD-style isolate-on-selection: when enabled (default), shapes/floors outside
  // the current selection scope dim. Toggled from the top HUD.
  const [isolateEnabled, setIsolateEnabled] = useState(true);
      ...
      case 'lShape': {
        ...
        if (p.points && p.points.length >= 3 && p.isClosed !== false) {
          try {
            ...
            console.warn('[ThreeDViewer] L freehand render failed', err);
          }
          break;
        }
      ...
      case 'tShape': {
        ...
        if (p.points && p.points.length >= 3 && p.isClosed !== false) {
          try {
            ...
            console.warn('[ThreeDViewer] T freehand render failed', err);
          }
          break;
        }
  ...
  useEffect(() => {
    ...
    const initScene = () => {
      ...
      const handleMouseDown = (event: MouseEvent) => {
        ...
        if (dr.tool !== 'pan') {
          // 變形 handle hit-test runs BEFORE the shape hit-test — handles sit
          // on top of geometry and a click should grab the handle, not the
          // shape underneath. On hit: bake the shape (if not already baked),
          // snapshot the current bbox, enter live drag. handlePointerMove
          // then drives the cage extent + applyBboxDeform.
          if (dr.tool === 'deform' && deformGroupRef.current) {
            raycasterRef.current.setFromCamera(mouseRef.current, cameraRef.current);
            const hHits = raycasterRef.current.intersectObjects(deformGroupRef.current.children, true);
            for (const h of hHits) {
              const ud = h.object.userData;
              if (ud?.deformHandle) {
                event.stopPropagation();
                // OrbitControls would otherwise start a rotate on drag — disable until pointerup.
                if (controlsRef.current) controlsRef.current.enabled = false;
                // Bake the shape group if it hasn't been baked yet, persisting to
                // shape.params.bakedGeometry so subsequent renders use it.
                const shapeGroup = (() => {
                  let g: THREE.Object3D | null = null;
                  objectsGroupRef.current?.traverse(o => { if (o.userData?.shapeId === ud.shapeId) g = g || o; });
                  return g as THREE.Group | null;
                })();
                if (!shapeGroup) break;
                const liveFloors = floorsRef.current ?? [];
                const owningFloor = liveFloors.find(f => f.shapes.some(s => s.id === ud.shapeId));
                const liveShape = owningFloor?.shapes.find(s => s.id === ud.shapeId);
                if (!owningFloor || !liveShape) break;
                let baked = liveShape.params.bakedGeometry;
                if (!baked) {
                  baked = bakeShapeGroup(shapeGroup);
                }
                // Reset originalBboxXZ to the CURRENT world bbox at the
                // start of EVERY drag. Otherwise, after the first drag the
                // baked positions are remapped but originalBboxXZ stays
                // stale → the second drag normalizes already-remapped
                // positions through the original denominator, producing
                // wrong scale + direction. Fresh denominator each drag =
                // each drag behaves like the first.
                const worldBox = new THREE.Box3().setFromObject(shapeGroup);
                const currentBbox = { minX: worldBox.min.x, maxX: worldBox.max.x, minZ: worldBox.min.z, maxZ: worldBox.max.z };
                baked = { ...baked, originalBboxXZ: currentBbox };
                deformActiveRef.current = {
                  shapeId: ud.shapeId,
                  floorId: ud.floorId,
                  axis: ud.deformHandle,
                  startBboxXZ: currentBbox,
                  startCursorXZ: (() => {
                    const cam = cameraRef.current!;
                    const dom = rendererRef.current!.domElement;
                    const proj = groundRef.current.projectRaw(event as PointerEvent, dom, cam);
                    return proj ?? { x: 0, z: 0 };
                  })(),
                };
                // Persist the (possibly newly-baked or denominator-refreshed)
                // snapshot so subsequent pointermove updates use it.
                const newFloors = liveFloors.map(f => f.id !== owningFloor.id ? f : ({
                  ...f,
                  shapes: f.shapes.map(s => s.id !== ud.shapeId ? s : ({ ...s, params: { ...s.params, bakedGeometry: baked } })),
                }));
          ...
          // Find hit shape. The active-floor filter only applies during the
          // move tool's phase-2/phase-3 sub-states (so the in-progress move
          // can't accidentally cross floors). For all other tools — including
          // delete / select / brush / rotate, and the move tool's initial
          // click — any shape on any floor is fair game; the user can see
          // exactly what they're clicking.
          raycasterRef.current.setFromCamera(mouseRef.current, cameraRef.current);
          ...
          const activeFloorIdForHit = stFloorId;  // null = no floor restriction
      ...
      const handleMouseMove = (event: MouseEvent) => {
        ...
        // === 變形 cage drag: re-project cursor onto floor plane, derive the
        // new bbox extent on the axis being dragged, run applyBboxDeform, and
        // write the result back to shape.params.bakedGeometry so the renderer
        // picks it up on the next frame. ===
        if (deformActiveRef.current) {
          const cam0 = cameraRef.current!;
          const dom0 = rendererRef.current!.domElement;
          const proj = groundRef.current.projectRaw(event as PointerEvent, dom0, cam0);
          if (!proj) return;
          const da = deformActiveRef.current;
          // Single-axis lock: ONLY the cursor component along the handle's
          // normal direction is consumed; the off-axis component is dropped
          // before quantization so it can't even contribute floating-point
          ...
          // Project onto the active axis — off-axis component explicitly zeroed.
          const isXAxis = da.axis === '+x' || da.axis === '-x';
          ...
          // -X handle in the -X world direction → -X edge moves -X (cage
          ...
          const sh = f?.shapes.find(s => s.id === da.shapeId);
          if (!f || !sh || !sh.params.bakedGeometry) return;
          const deformed = applyBboxDeform(sh.params.bakedGeometry, newBbox);
          const newFloors = live.map(ff => ff.id !== f.id ? ff : ({
            ...ff,
            shapes: ff.shapes.map(s => s.id !== sh.id ? s : ({ ...s, params: { ...s.params, bakedGeometry: deformed } })),
          }));
          onFloorsChange?.(newFloors);
          return;
        }
        ...
        setCursor(world);
        if (dr.tool !== 'pan') {
          if (dr.state.kind === 'extruding' && event.movementY) {
            dr.handleExtrudeDrag(event.movementY);
          }
      ...
      const handleMouseUp = (event: MouseEvent) => {
        // Release 變形 drag: re-enable orbit controls + clear active handle.
        if (deformActiveRef.current) {
          deformActiveRef.current = null;
  ...
  // External layer visibility:
  //   主體模型 (mainModel) → all shape groups in objectsGroupRef
  //   平面    (planes)    → the per-floor XZ working plane (gridHelper + snapDots)
  // Re-runs when shapes are rebuilt (via `floors` dep) so newly-added shapes
  // inherit the current layer toggle state immediately.
  useEffect(() => {
    const group = objectsGroupRef.current;
    const mainModelVisible = layerVisibility?.mainModel !== false;
    const planesVisible    = layerVisibility?.planes    !== false;
    if (group) {
      // Top-level children carry layerKey for shape groups; per-floor workplanes
      // live nested inside floorGroups, so deep-walk too.
      for (const child of group.children) {
        const k = (child as THREE.Object3D).userData?.layerKey as 'mainModel' | 'planes' | undefined;
        if (k === 'mainModel') (child as THREE.Object3D).visible = mainModelVisible;
      }
      group.traverse((obj) => {
        const k = obj.userData?.layerKey as 'mainModel' | 'planes' | undefined;
        if (k === 'planes') obj.visible = planesVisible;
      });
    }
    if (gridHelperRef.current)   gridHelperRef.current.visible   = planesVisible;
    if (snapDotsGroupRef.current) snapDotsGroupRef.current.visible = planesVisible;
  }, [layerVisibility, sceneReady, floors]);
  ...
  useEffect(() => {
    if (draw.tool === 'polyline' && !draw.polyline3D) {
      if (!topViewLocked) {
        topViewWasUserToggledRef.current = false;
        setTopViewLocked(true);
      }
    } else {
      // If we auto-locked (not user-toggled), restore on tool exit OR when the
      // user flips on 3D polyline mode.
      if (topViewLocked && !topViewWasUserToggledRef.current) {
        setTopViewLocked(false);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [draw.tool, draw.polyline3D]);
  ...
  useEffect(() => {
    ...
    if (scene) {
      ...
      // Honor the 平面 toggle on rebuild — otherwise a fresh GridHelper appears
      // visible regardless of the user having hidden the layer.
      grid.visible = (layerVisibility?.planes !== false);
  ...
  // 變形 cage + handles overlay. Active when the 變形 tool is selected AND a
  // shape is selected. Builds a wireframe box around the shape's current XZ
  // bbox at its base Y, with 4 sphere handles at the +X / -X / +Z / -Z
  // midpoints. Handles are raycast-targetable (userData.deformHandle = axis)
  // and the cage moves with floor / selection / shape-bbox changes.
  useEffect(() => {
    const scene = sceneRef.current;
    if (!scene) return;
    // Tear down any prior overlay.
    if (deformGroupRef.current) {
      scene.remove(deformGroupRef.current);
      deformGroupRef.current.traverse((o: any) => { o.geometry?.dispose?.(); if (o.material && !Array.isArray(o.material)) o.material.dispose?.(); });
      deformGroupRef.current = null;
    }
    // Tear down any prior vertex-normal arrows.
    for (const h of normalsHelpersRef.current) {
      scene.remove(h);
      (h as any).geometry?.dispose?.();
      (h as any).material?.dispose?.();
    }
    normalsHelpersRef.current = [];

    if (draw.tool !== 'deform' || !selectedShapeId || !floors) return;

    // Find the floor that owns the selected shape, and its mesh group in the
    // scene (so we can read the live bbox from the rendered geometry).
    let owningFloorId: string | null = null;
    for (const f of floors) {
      if (f.shapes.some(s => s.id === selectedShapeId)) { owningFloorId = f.id; break; }
    }
    if (!owningFloorId) return;
    const objs = objectsGroupRef.current;
    if (!objs) return;
    let shapeGroup: THREE.Object3D | null = null;
    objs.traverse(o => { if (o.userData?.shapeId === selectedShapeId) shapeGroup = shapeGroup || o; });
    if (!shapeGroup) return;
    // Compute bbox in world space (cage sits at world-space coords, not parented to shape).
    const box = new THREE.Box3().setFromObject(shapeGroup as THREE.Object3D);
    if (!isFinite(box.min.x)) return;
    const baseY = computeFloorBaseY(floors, owningFloorId);

    const grp = new THREE.Group();
    scene.add(grp);
    deformGroupRef.current = grp;

    // Wireframe cage — XZ rectangle at baseY (top edge at baseY + tallest shape
    // height isn't needed for an XZ-only cage).
    const cageMat = new THREE.LineBasicMaterial({ color: 0xf59e0b, transparent: true, opacity: 0.9 });
    const cagePts = [
      new THREE.Vector3(box.min.x, baseY + 0.05, box.min.z),
      new THREE.Vector3(box.max.x, baseY + 0.05, box.min.z),
      new THREE.Vector3(box.max.x, baseY + 0.05, box.max.z),
      new THREE.Vector3(box.min.x, baseY + 0.05, box.max.z),
      new THREE.Vector3(box.min.x, baseY + 0.05, box.min.z),
    ];
    const cageGeo = new THREE.BufferGeometry().setFromPoints(cagePts);
    const cageLine = new THREE.Line(cageGeo, cageMat);
    grp.add(cageLine);

    // 4 spherical handles, each tagged with the axis it edits.
    const handleGeo = new THREE.SphereGeometry(0.8, 16, 12);
    ...
    const handles: Array<{ pos: THREE.Vector3; axis: '+x'|'-x'|'+z'|'-z' }> = [
      { pos: new THREE.Vector3(box.max.x, baseY + 0.05, midZ), axis: '+x' },
      { pos: new THREE.Vector3(box.min.x, baseY + 0.05, midZ), axis: '-x' },
      { pos: new THREE.Vector3(midX, baseY + 0.05, box.max.z), axis: '+z' },
      { pos: new THREE.Vector3(midX, baseY + 0.05, box.min.z), axis: '-z' },
    ];
    for (const h of handles) {
      const m = new THREE.Mesh(handleGeo, handleMat);
      m.position.copy(h.pos);
      m.renderOrder = 999;
      m.userData = { deformHandle: h.axis, shapeId: selectedShapeId, floorId: owningFloorId };
      grp.add(m);
    }

    // Vertex-normal arrows: one VertexNormalsHelper per child Mesh of the
    // selected shape group. Arrow length scales with shape extent so it's
    // visible without overwhelming the view.
    const extent = Math.max(box.max.x - box.min.x, box.max.z - box.min.z, 1);
    const arrowLen = Math.max(0.4, Math.min(2.5, extent * 0.05));
    (shapeGroup as THREE.Object3D).traverse(o => {
      if (o instanceof THREE.Mesh && (o.geometry as THREE.BufferGeometry).getAttribute('normal')) {
        const helper = new VertexNormalsHelper(o, arrowLen, 0x10b981);  // emerald
        scene.add(helper);
        normalsHelpersRef.current.push(helper);
      }
    });
  }, [draw.tool, selectedShapeId, floors, sceneReady]);
  ...
  useEffect(() => {
    ...
    if (floors && floors.length > 0) {
      ...
      // a floor is in edit mode), dim everything *outside* the selection scope
      // so the active item stands out. Gated by the HUD 隔離 toggle — when off,
      // every shape renders at full opacity regardless of selection.
      const hasShapeSelection = selectedShapeId != null;
      const hasIsolation     = isolateEnabled && (editingFloorId != null || selectedFloorId != null || hasShapeSelection);
      ...
      floors.forEach((floor, floorIndex) => {
        ...
        // Floor is "in scope" if it's the editing target, OR the selected floor.
        // If only a shape is selected (no floor explicitly), use that shape's floor.
        const shapeOwningFloor = hasShapeSelection
          ? floors.find(f => f.shapes.some(s => s.id === selectedShapeId))?.id
          : null;
        const isFloorInScope = isEditingFloor || isSelectedFloor || floor.id === shapeOwningFloor;
        const isDimmedFloor  = hasIsolation && !isFloorInScope;
        ...
        // Floor slab (counts as part of the 平面 layer — it's the horizontal
        // working surface of each floor; users hide them together with the
        // workplane grids via the 平面 toggle).
        const slab = createFloorSlab(maxW, maxD, 0, isSelectedFloor && !isNonEditingInEditMode);
        slab.userData = { floorId: floor.id, layerKey: 'planes' };
        slab.visible = (layerVisibility?.planes !== false);
        if (isNonEditingInEditMode || isDimmedFloor) {
          slab.traverse((child) => {
        ...
        // Per-floor XZ working plane (visual grid at floor base). Tagged
        // `layerKey: 'planes'` so the 平面 layer toggle hides/shows all of them
        // in one click.
        const planeSize = Math.max(maxBuildingW, maxBuildingD) * 1.5 + 20;
        const floorPlane = new THREE.GridHelper(planeSize, Math.max(10, Math.round(planeSize / gridSize)), 0x94a3b8, 0xcbd5e1);
        (floorPlane.material as THREE.LineBasicMaterial).transparent = true;
        (floorPlane.material as THREE.LineBasicMaterial).opacity = isFloorInScope ? 0.45 : 0.15;
        floorPlane.position.y = 0.01;  // sit just above slab to avoid z-fighting
        floorPlane.userData = { floorId: floor.id, layerKey: 'planes' };
        // Apply current visibility immediately — visibility useEffect runs
        // BEFORE this rebuild effect, so freshly-created planes would otherwise
        // appear visible regardless of the 平面 toggle state until next change.
        floorPlane.visible = (layerVisibility?.planes !== false);
        ...
        floor.shapes.forEach((shape, shapeIndex) => {
          ...
          // Per-shape extrusion height: shapes that have been individually
          // extruded (via 拉伸 tool or the panel) use their own params.height /
          // extrudeHeight; untouched shapes inherit floor.floorHeight.
          ...
          // Same fix as the per-floor plane: apply current visibility immediately
          // so newly-built shapes honor the 主體模型 toggle on the first render.
          shapeGroup.visible = (layerVisibility?.mainModel !== false);
          ...
          // CAD isolate-mode dim:
          //   1. Shapes on dimmed floors → fade.
          //   2. Within the in-scope floor, if a specific shape is selected,
          //      fade the *sibling* shapes (not the selected one).
          const isDimmedShape =
            isNonEditingInEditMode ||
            isDimmedFloor ||
            (isolateEnabled && hasShapeSelection && !isSelectedShape);
          if (isDimmedShape) {
            shapeGroup.traverse((child) => {
              if (child instanceof THREE.Mesh) {
          ...
          floorGroup.add(shapeGroup);
            ...
            console.warn('[ThreeDViewer] shape render failed', shape.type, shape.id, err);
          }
        });
        ...
        yOffset += effectiveFloorHeight(floor);
      });
  ...
  }, [objects, floors, selectedFloorId, selectedShapeId, editingFloorId, sceneReady, isolateEnabled, layerVisibility]);
  ...
  const toolbar = (
    <DrawingToolbar
      ...
      variant={inlineToolbar ? 'inline' : 'overlay'}
    />
  );

  const canvasBlock = (
    <div ref={containerRef} className={`${inlineToolbar ? 'flex-1 min-w-0' : 'w-full'} h-full relative overflow-hidden bg-slate-50`}>
      {/* Orientation label — themed; matches the top-center HUD pill style */}
      <div
        className="absolute top-3 right-3 rounded-lg font-black pointer-events-none z-10 flex items-center whitespace-nowrap"
        style={{
      ...
      {/* In overlay mode (legacy), toolbar lives inside the canvas wrapper. */}
      ...
      <DrawingHUD
        ...
        onToggleIsolate={() => setIsolateEnabled(v => !v)}
        lang={lang}
      />
      ...
      <DrawingParamBar
        ...
        lang={lang}
        angleSnapDeg={draw.angleSnapDeg}
        onAngleSnapChange={draw.setAngleSnapDeg}
        onLengthPreview={draw.setPreviewLength}
        onInputFocusChange={draw.setInputFocused}
        polylineSegMode={draw.polylineSegMode}
        onPolylineSegModeChange={draw.setPolylineSegMode}
        polyline3D={draw.polyline3D}
        onPolyline3DChange={draw.setPolyline3D}
      />
      ...
      {/* Extrude height dialog disabled — useDrawingTool auto-commits with the
          active floor's height + default extras. Users tweak per-shape facade
          params via FloorManagerPanel after the shape is created. */}
  ...
  if (inlineToolbar) {
    return (
      <div className="w-full h-full flex flex-col" style={{ background: 'var(--color-bg)' }}>
        {/* Region header row: drawing-tools label + 3D platform label */}
        <div
          className="flex items-stretch flex-shrink-0"
          style={{ borderBottom: '1px solid var(--color-border)' }}
        >
          <div
            className="flex flex-col items-center justify-center px-2 py-1.5 flex-shrink-0"
            style={{
              width: '54px',
            ...
            title="繪圖工具 / EVA-TOOLS"
          >
            <span style={{ fontSize: '11px', fontWeight: 800 }}>繪圖工具</span>
            <span style={{ fontSize: '9px', fontWeight: 700, opacity: 0.65, letterSpacing: '0.05em', marginTop: '2px' }}>EVA-TOOLS</span>
          </div>
          <div
          ...
          >
            <span style={{ fontSize: '13px', fontWeight: 800 }}>3D 繪圖平台</span>
            <span style={{ fontSize: '10px', fontWeight: 700, opacity: 0.7, letterSpacing: '0.05em' }}>EVA-3D</span>
          </div>
        </div>

        {/* Body: toolbar column + canvas */}
